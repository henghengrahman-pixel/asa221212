...
from poster import HARI_ID, BULAN_ID, generate_prediction_data, create_poster
...
async def scheduler():
    state = load_state()
    logging.info("Scheduler aktif. Timezone=%s, channel=%s", TZ.zone, ",".join(CHANNEL_IDS))
    while True:
        try:
            now = datetime.now(TZ)
...
async def cmd_start(message: types.Message):
    if not is_admin(message.from_user.id):
        ...
        return
    await message.reply(
        "✅ Bot prediksi gambar OMTOGEL aktif.\n\n"
...
async def cmd_preview(message: types.Message):
    ...
    with open(poster, "rb") as photo:
        await message.reply_photo(photo, caption=f"Preview <b>{pasaran}</b>")
...
async def on_startup(_):
    schedule = load_schedule()
    logging.info("Validasi startup OK: %d pasaran", len(schedule))
